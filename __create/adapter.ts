...
export default function NeonAdapter(client: Pool): NeonAdapter {
  return {
    ...
    async createUser(user: Omit<AdapterUser, 'id'> & { gender?: string, dob?: string, country?: string }) {
      const { name, email, emailVerified, image, gender, dob, country } = user;
      const sql = `
        INSERT INTO auth_users (name, email, "emailVerified", image, gender, dob, country)
        VALUES ($1, $2, $3, $4, $5, $6, $7)
        RETURNING id, name, email, "emailVerified", image, gender, dob, country`;
    ...
    async updateUser(user: Partial<AdapterUser> & { gender?: string, dob?: string, country?: string }): Promise<AdapterUser> {
      ...
      const { id, name, email, emailVerified, image, gender, dob, country } = newUser;
      const updateSql = `
        UPDATE auth_users set
        name = $2, email = $3, "emailVerified" = $4, image = $5, gender = $6, dob = $7, country = $8
        where id = $1
        RETURNING name, id, email, "emailVerified", image, gender, dob, country
      `;
